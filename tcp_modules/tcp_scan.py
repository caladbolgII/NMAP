...
def tcp_scan(ip_addr, port, delay):
	...
	print ("Scan report for: {}".format(socket.getfqdn(ip_num)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a','--hosts' ,help='Hosts',required=True)
    ...
    if args.hosts:
	try:
		ip, endip = args.hosts.split('-')
		print ip,endip
		ip_mask, startip = ip.rsplit('.',1)
		print "mask",ip_mask, startip
		ipspan = int(endip)-int(startip)+1
		print "ipspan",ipspan
		ip_addr.insert(0,ip_mask+'.'+startip)
		print "assemble",ip_mask+'.'+startip
		for index in range(1,ipspan):
			ip_addr.insert(index,ip_mask+'.'+str(int(startip)+index))
			print "assemble",index,ip_mask+'.'+str(int(startip)+index)
		print port
	except ValueError:
		try:
			ip, subnet = args.hosts.split('/')
			print ip,subnet
			network = ipaddress.ip_network(unicode(args.hosts))
			for x in network.hosts():
				ip_addr.insert(0,str(x))

		except ValueError:
			ip_addr.insert(0,str(args.hosts))
			print ip_addr
    		#network,subnet = args.net.split('/')
    if args.udp:
   	tcp = 0
    if args.tcp:
    	tcp = 1
	...
	try:
		...
		port.insert(0,int(start))
		span = int(end)-int(start)+1
		for index in range(1,span):
			port.insert(index,int(start)+index)
		print port
	except ValueError:
		port.insert(0,int(args.port))
    #port = range(1, 30)
    #ports of standard 1k.
